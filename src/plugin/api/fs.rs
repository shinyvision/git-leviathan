...
//! array-like table of `{name, path, is_dir, is_symlink, size, modified}`
//! entries, pre-sorted (dirs first, then case-sensitive name). `is_symlink`
//! is detected without following the link, mirroring `fs.is_symlink`; a
//! symlink to a directory still reports `is_dir = false` so the two flags
//! never both hold.
//!
//! `read_file(path)` reads a UTF-8 text file and returns `(content, nil)`
//! on success, `(nil, err)` otherwise. Capped at 8 MiB so a plugin can't
//! stall the UI thread on a runaway log; plugins that need bigger payloads
//! should chunk their own reads outside the Lua call.
//!
//! `write_file(path, content)` writes `content` to `path`, replacing any
//! existing file, and returns `(true, nil)` on success or `(false, err)`
//! when the write fails. Mirrors `read_file`'s 8 MiB cap on the payload
//! so a runaway plugin can't blow up the disk from a single Lua call.
//! Parent directories are not created — plugins that need them should
//! call `mkdir` first.
//!
//! `mkdir(path)` creates `path` and any missing parent directories,
//! returning `(true, nil)` on success or `(false, err)` on failure.
//! Existing directories at `path` are treated as success, matching the
//! `create_dir_all` shape — plugins use it to prepare a target before
//! `write_file`.
//!
//! `parent(path)`, `basename(path)`, `stem(path)`, and `extension(path)`
//! are pure path-string helpers that never touch the filesystem. They
//! return nil when the path has no such component (root has no parent;
//! a path ending in `..` has no file name; a name with no `.` or a
//! leading-dot dotfile has no extension). `stem` returns the file name
//! up to (but not including) the final `.`, or the whole file name when
//! there is no extension — `archive.tar.gz` → `archive.tar`,
//! `.gitignore` → `.gitignore`, `README` → `README`.
//!
//! `is_symlink(path)` returns true only when `path` is itself a symlink
//! (uses `symlink_metadata`, so it does not follow). Returns false for
//! regular files, directories, broken paths, or permission errors —
//! matching the boolean shape of `is_file`/`is_dir`.
//!
//! `canonicalize(path)` resolves `.`, `..`, and symlinks into an
//! absolute path, returning `(path, nil)` on success or `(nil, err)`
//! when the path does not exist or cannot be accessed. Mirrors the
//! result-or-error shape of `read_file` and `list_dir`.
//!
//! `cwd()` returns the process's current working directory as
//! `(path, nil)`, or `(nil, err)` when the directory was deleted or
//! is otherwise inaccessible. Complements `home()` so plugins can
//! resolve relative user input or display the launch directory.
//!
//! `config_dir()` returns the user's standard per-user configuration
//! directory (`$XDG_CONFIG_HOME` or `~/.config` on Linux, `%APPDATA%` on
//! Windows, `~/Library/Application Support` on macOS), or nil when the
//! platform exposes no such location. Plugins use it to persist per-user
//! state at the OS-standard place without rebuilding the platform path
//! from `home()`. Mirrors `home()`'s nil-on-unresolved shape.
//!
//! `cache_dir()` returns the user's standard per-user cache directory
//! (`$XDG_CACHE_HOME` or `~/.cache` on Linux, `%LOCALAPPDATA%` on Windows,
//! `~/Library/Caches` on macOS), or nil when the platform exposes no such
//! location. Plugins use it for throwaway derived state (rendered diffs,
//! decoded blobs) that the OS may evict, distinct from `config_dir`'s
//! durable per-user settings. Mirrors `config_dir`'s nil-on-unresolved
//! shape.
//!
//! `data_dir()` returns the user's standard per-user data directory
//! (`$XDG_DATA_HOME` or `~/.local/share` on Linux, `%APPDATA%` on Windows,
//! `~/Library/Application Support` on macOS), or nil when the platform
//! exposes no such location. Plugins use it for durable per-user data
//! the OS must not evict (sqlite stores, generated exports, history),
//! distinct from `config_dir`'s settings and `cache_dir`'s evictable
//! derivatives. Mirrors `config_dir`'s nil-on-unresolved shape.
//!
//! `state_dir()` returns the user's standard per-user state directory
//! (`$XDG_STATE_HOME` or `~/.local/state` on Linux), or nil on Windows
//! and macOS where the platform exposes no equivalent. Plugins use it
//! for state that should survive restarts but isn't synchronised across
//! machines (recently-opened lists, undo history, session logs) —
//! distinct from `data_dir`'s portable user data and `cache_dir`'s
//! evictable derivatives. Mirrors `config_dir`'s nil-on-unresolved
//! shape; on platforms without a state directory plugins should fall
//! back to `data_dir`.
//!
//! `temp_dir()` returns the process's temporary directory as a string,
//! resolved via `$TMPDIR` / `$TMP` / `$TEMP` (platform-specific) with a
//! sensible fallback (`/tmp` on Unix, the system temp on Windows). The
//! path is always returned — the OS guarantees a value, though the
//! directory itself may have been deleted at runtime; plugins that
//! intend to write should pair with `mkdir`. Rounds out the
//! `home`/`cwd`/`temp_dir` trio so plugins have all three standard
//! location anchors without shelling out.
//!
//! `is_absolute(path)` is a pure path-string check: true when the path
//! has a root component (`/foo` on Unix, `C:\foo` on Windows), false
//! otherwise. Plugins use it to validate user-supplied paths before
//! `read_file`/`list_dir`/`canonicalize`, or to decide whether a path
//! still needs `fs.join` against a base directory.
//!
//! `size(path)` returns `(bytes, nil)` on success or `(nil, err)` when
//! the path is missing or inaccessible. Uses `symlink_metadata` so a
//! symlink reports the link's own size, matching `list_dir`'s per-entry
//! `size`. For directories the underlying filesystem decides the value,
//! so plugins should branch on `is_dir` first if they care.
//!
//! `relative_to(path, base)` is a pure path-string helper that returns
//! `path` rewritten relative to `base` when `path` is a descendant of
//! `base`, or nil otherwise. Plugins use it to display file paths
//! relative to a repository root or workspace without doing string
//! arithmetic. No filesystem access; comparison is component-wise so
//! `..`/`.` segments are not collapsed — canonicalize first if both
//! sides may contain them.
//!
//! `with_extension(path, ext)` is a pure path-string helper that
//! returns `path` with its final extension replaced by `ext`, added
//! when there is none, or stripped when `ext` is empty. Returns nil
//! when `path` has no file-name component (root, empty, or trailing
//! `..`), matching the nil-on-no-component shape of `basename`/`stem`.
//! Only the final extension is touched — `archive.tar.gz` + `bz2`
//! becomes `archive.tar.bz2`.
//!
//! `with_file_name(path, name)` is a pure path-string helper that
//! returns `path` with its final component replaced by `name`. Returns
//! nil when `path` has no file-name component (root, empty, or trailing
//! `..`), matching `with_extension`. Plugins use it to rename a path in
//! place without filesystem access — `/home/user/file.txt` + `notes.md`
//! becomes `/home/user/notes.md`.
//!
//! `modified(path)` returns `(unix_seconds, nil)` on success or
//! `(nil, err)` when the path is missing or inaccessible. Uses
//! `symlink_metadata` so a symlink reports the link's own mtime,
//! matching the no-follow behavior of `size` and `list_dir`'s per-entry
//! `modified`. Plugins use it for freshness checks without enumerating
//! the parent directory.
//!
//! `read_link(path)` returns `(target, nil)` when `path` is a symlink,
//! `(nil, err)` otherwise. The returned target is the literal stored
//! path (may be relative to `path`'s parent) — it is not followed,
//! resolved, or canonicalized, complementing `is_symlink`'s no-follow
//! shape. Plugins that want a usable absolute path should pass the
//! result through `canonicalize` (or `join` with `parent(path)` when the
//! target is relative).
//!
//! `rename(src, dst)` moves or renames `src` to `dst` and returns
//! `(true, nil)` on success or `(false, err)` on failure. Mirrors
//! `std::fs::rename`: atomic when both paths live on the same
//! filesystem, overwrites an existing regular file at `dst`, but fails
//! across filesystems (plugins should fall back to `read_file` +
//! `write_file` + `delete` for cross-device moves). Pairs with
//! `write_file`/`delete`/`mkdir` to complete the read/write/remove/move
//! primitive set.
//!
//! `copy(src, dst)` duplicates `src` to `dst` and returns `(true, nil)`
//! on success or `(false, err)` on failure. Mirrors `std::fs::copy`:
//! works across filesystems and overwrites an existing regular file at
//! `dst`, unlike `rename`. Source size is capped at 8 MiB so a plugin
//! can't stall the UI thread on a runaway copy, matching the
//! `read_file`/`write_file` cap — plugins that need bigger payloads
//! should stream their own copy outside the Lua call.
//!
//! `metadata(path)` returns `(table, nil)` on success or `(nil, err)`
//! when the path is missing or inaccessible. The table mirrors a single
//! `list_dir` entry exactly — `{name, path, is_dir, is_symlink, size,
//! modified}` — so plugins can run the same per-entry code over either
//! a directory listing or a single explicit path. Uses `symlink_metadata`,
//! matching `list_dir`'s no-follow behavior: a symlink reports
//! `is_symlink = true` and `is_dir = false` even if its target is a
//! directory. Pairs with the per-field accessors (`size`, `modified`,
//! `is_dir`, `is_symlink`) when one batched call beats several.
//!
//! `append_file(path, content)` appends `content` to `path`, creating
//! the file when it does not exist, and returns `(true, nil)` on
//! success or `(false, err)` on failure. The appended payload is
//! capped at 8 MiB per call, matching `write_file` — plugins that
//! need bigger payloads should chunk their own appends outside the
//! Lua call. Pairs with `write_file`/`read_file` so plugins can keep
//! a log or journal without read+write round-trips.
//!
//! `read_lines(path)` returns `(table, nil)` on success — an
//! array-like table of lines without their terminators — or
//! `(nil, err)` on failure. Splits on `\n` and `\r\n` (matches
//! `str::lines`), so plugins parsing line-oriented files
//! (`.gitignore`, `.gitattributes`, license text) don't trip on CRLF
//! sources or trailing newlines the way a Lua-side `gmatch` pattern
//! does. Same 8 MiB cap as `read_file`.
//!
//! `touch(path)` creates `path` as an empty file when it does not
//! exist and updates its modification time to "now" when it does,
//! returning `(true, nil)` on success or `(false, err)` on failure.
//! Matches the Unix `touch(1)` shape so plugins can drop sentinel /
//! marker files (lockfiles, "did you see this?" beacons) and bump
//! existing files' mtime without rewriting their contents. Parent
//! directories are not created — plugins that need them should call
