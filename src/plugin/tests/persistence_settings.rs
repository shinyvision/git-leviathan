...
id = "persistence_settings"
name = "persistence_settings"
version = "0.1.0"
api_version = "1.0"
"#;
...
fn surface_path(host: &MockHost, surface: &str) -> PathBuf {
    ...
    PathBuf::from(
        snap.storage
            .iter()
            .find(|row| row.plugin_id == "persistence_settings" && row.surface == surface)
...
fn corrupt_state_is_backed_up_and_recreated() {
    let mut host = MockHost::new();
    host.load_inline("persistence_settings", MANIFEST, "")
        .expect("load");
    let state_file = surface_path(&host, "state").join("data.json");
    ...
    assert_eq!(
        host.read_global_i64("persistence_settings", "missing"),
        Some(1)
    );
...
fn settings_validate_before_save_and_fire_on_change() {
    ...
    assert_eq!(
        host.read_global_i64("persistence_settings", "schema_ok"),
        Some(1)
    );
    assert_eq!(
        host.read_global_string("persistence_settings", "default_mode")
            .as_deref(),
        Some("auto")
    );
    assert_eq!(
        host.read_global_i64("persistence_settings", "bad_ok"),
        Some(0)
    );
    assert!(host
        .read_global_string("persistence_settings", "bad_err")
        .unwrap()
        .contains("limit"));
    assert_eq!(
        host.read_global_i64("persistence_settings", "good_ok"),
        Some(1)
    );
    assert_eq!(
        host.read_global_i64("persistence_settings", "changed_limit"),
        Some(4)
    );
...
fn reset_surfaces_and_secret_metadata_do_not_expose_secret_values() {
    ...
    host.reset_plugin_storage("persistence_settings", "cache")
        .expect("reset cache");
    let snap = host.introspect();
    let cache = snap
        .storage
        .iter()
        .find(|row| row.plugin_id == "persistence_settings" && row.surface == "cache")
        .unwrap();
    assert!(!cache.exists);
    ...
    host.reset_plugin_storage("persistence_settings", "secrets")
...
fn persist_transaction_rolls_back_on_callback_failure() {
    ...
    assert_eq!(
        host.read_global_i64("persistence_settings", "failed"),
        Some(1)
    );
    assert_eq!(
        host.read_global_i64("persistence_settings", "a_missing"),
        Some(1)
    );
    assert_eq!(host.read_global_i64("persistence_settings", "b"), Some(2));
}
