//! Phase 6 transactional reload tests.
//!
...
fn staged_reload_capability_widening_emits_audit_diagnostic() {
    ...
    // `reload.capability_widened` info diagnostic. Phase 10 will
    // upgrade this into a hard `capability.staging_denied` failure
...
"#,
    ...
    .expect("widening reload should succeed (Phase 10 will gate it)");
    assert!(host
        .diagnostics()
