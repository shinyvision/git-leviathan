//! command registry acceptance tests: typed commands, command palette
...
fn lua_command_create_then_invoke_through_lua_and_rust_share_dispatcher() {
    ...
    //   1. the Rust `host.invoke_command(...)` command registry entry point
...
fn failed_reload_keeps_previous_command_active() {
    ...
    // rollback test in autocmd_events.
    let mut host = MockHost::new();
    host.load_inline(
        "tx",
        &manifest("tx"),
...
fn command_executed_event_fires_after_success_and_failure() {
    // autocmd typed event subscription. We register two autocmd
