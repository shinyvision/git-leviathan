//! Phase 9 acceptance tests: context-aware keymaps.
//!
//! Each test cites the acceptance-criterion bullet from the plan
//! (`PLUGIN_REFACTOR.md` Phase 9, lines ~534-577) it covers. The
//! ordering follows the "Required tests" list in the Phase 9 brief.
...
#[test]
fn plugin_keymap_routes_through_phase8_command_dispatcher() {
    // Acceptance: at least one Phase 8 command can be invoked
    // through a Phase 9 keymap. The command dispatcher records every
    ...
    host.load_inline(
        ...
        leviathan.command.create("p.refresh", {
            description = "Phase 8 command driven by Phase 9 keymap",
            run = function(args)
    ...
    // Phase 8 dispatcher ran the body.
    let post = host
        .command_registry()
        .borrow()
        .summaries()
