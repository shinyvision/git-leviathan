//! Phase 24 final acceptance tests. These tests load the fixture plugins
//! under `plugins/phase24_fixtures` and assert against the host registries
...
fn fixtures_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("plugins/phase24_fixtures")
}
...
fn stage_good_fixtures(host: &MockHost) -> HashMap<String, PathBuf> {
    let root = host.root().join("phase24_good");
    fs::create_dir_all(&root).expect("phase24 good root");
...
fn copy_bad_fixture(host: &MockHost, name: &str) -> PathBuf {
    let dst = host.root().join("phase24_bad").join(name);
...
fn good_demo_plugins_exercise_dream_system_and_cleanly_unload() {
    ...
    std::thread::sleep(Duration::from_millis(50));
    {
        ...
        writeln!(file, "phase24").expect("write watched file");
        file.sync_all().ok();
    }
    drain_until(
        &mut host,
    ...
    let (repo_dir, repo) = init_test_repo("phase24_repo_guard");
...
fn bad_plugin_suite_is_contained_and_inspectable() {
    ...
    let bad_root = host.root().join("phase24_bad");
    host.host_mut()
