...
pub(crate) fn view(slide_offset: f32) -> Element<'static, Message> {
    let label = text("Do you want to immediately commit the cherry picked changes?")
        .size(theme::FONT_SM)
    ...
    let yes_btn = overlay_button(
        "Yes",
        CREATE_BUTTON,
        RepositoryMessage::OverlayPanel(OverlayPanelAction::CherryPickImmediateConfirmed),
    );
    let no_btn = overlay_neutral_button(
        "No",
        RepositoryMessage::OverlayPanel(OverlayPanelAction::CherryPickStagedConfirmed),
    );
    let cancel_btn = overlay_button(
        "Cancel",
        DANGER_BUTTON,
        RepositoryMessage::OverlayPanel(OverlayPanelAction::CherryPickCanceled),
    );
